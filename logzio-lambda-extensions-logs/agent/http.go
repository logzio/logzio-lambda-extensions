...
func NewHttpAgent(logzioLogger *logzio.LogzioSender, jq *queue.Queue) (*HttpAgent, error) {
...
func (a HttpAgent) Init(agentID string) error {
	...
	eventTypes := getEventTypesToRegister()
...
func getEventTypesToRegister() []logsapi.EventType {
	eventTypes := []logsapi.EventType{logsapi.Function}

	enablePlatformLogs := utils.GetEnablePlatformLogs()
